...
The OpenCode plugin and Claude Code hooks invoke `~/Library/Application Support/NotchBot/bin/notchbot-hook`. The helper selects lifecycle state, source, session identifier, working-directory path, terminal identifier, reason, expiry, a bounded task label, and, only when the user opts in, an assistant-response excerpt of at most 240 characters. It authenticates and encrypts the event to `~/Library/Application Support/NotchBot/notchbot.sock` using the same-user key at `~/Library/Application Support/NotchBot/event.key`. OpenCode labels come from its session title or project metadata. Claude Code labels come from the existing `session_title`, `task_subject`, or `agent_type` hook fields, falling back to the project-directory basename. NotchBot does not select prompt fields, transcripts, task descriptions, or response excerpts to construct labels, but the integrated agent may have generated a title or subject from conversation content before exposing that metadata. Labels appear in the hover queue, remain in process memory, and are not written to disk by NotchBot. Response excerpts are disabled by default, remain in process memory, are not written to disk by NotchBot, and expire from the app after 15 minutes. NotchBot uses the working-directory path without traversing or reading arbitrary project files.
...
NotchBot 0.2.1 is not App Sandbox enabled and requests no signing entitlements. App Sandbox adoption is deferred to v0.3.0.

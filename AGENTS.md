...
This is a Swift Package Manager project with four main targets:

- `NotchBot`: SwiftUI/AppKit menu-bar application, panels, preferences, notifications, and integration installer.
- `notchbot-hook`: provider-facing helper that decodes bounded input and sends authenticated local events.
...
- `Sources/NotchBot/IntegrationInstaller.swift`: guarded integration installation and migration.
- `Sources/NotchBotIntegrationCore/`: provider integration generation and decoding.
- `Sources/NotchBotHook/main.swift`: helper executable entry point and permission-response handling.
...
- `Sources/NotchBotHook/main.swift`
- `Sources/NotchBot/IntegrationInstaller.swift`

...
Version 0.9.0 supports OpenCode and Claude Code on Apple Silicon Macs running macOS 14 or later.

## Install

Download `NotchBot-0.9.0.dmg` and `NotchBot-0.9.0.dmg.sha256` from the [latest GitHub release](https://github.com/adamdavies/NotchBot/releases/latest), verify the checksum, open the DMG, and move NotchBot into `/Applications`.

```sh
shasum -a 256 -c NotchBot-0.9.0.dmg.sha256
```
...
Open the fixed robot menu-bar icon and select **Install Integrations** for a first installation. Once integrations are installed and current, that entry reads **Reinstall Integrations** and rewrites the managed files in place. Version 0.9.0 raises the integration revision to 20. After installing this release, select **Update Integrations** and restart every running OpenCode and Claude Code session.
...
Version 0.9.0 uses integration revision 20 and retains event protocol version 4, which gains an optional context-window usage field. Integration revision 19 files from v0.6.0 through v0.8.0 are recognized and migrated, but this release does require selecting **Update Integrations** and restarting running Claude Code and OpenCode sessions.
...
NotchBot 0.9.0 is not App Sandbox enabled and requests no signing entitlements.
